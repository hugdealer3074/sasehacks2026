...
# ClinicEntry(name="UHealth Lennar Foundation Medical Center",
...
# ),
# ClinicEntry(
# name="Coral Gables Hospital - Emergency Room",
...
#         # No more dict indexing errors! Just use dot notation.
...
#         }
#         all_scored_clinics.append(clinic_entry)
...
def clean_transcript(text: str) -> str:
    if not text:
        return text
    text = re.sub(r"\([^)]*\)", "", text)
    filler_words = [r"\bum\b", r"\buh\b", r"\bmmm+\b", r"\beh\b", r"\beste\b", r"\bah\b"]
    for filler in filler_words:
        text = re.sub(filler, "", text, flags=re.IGNORECASE)
    text = re.sub(r"[^a-zA-ZáéíóúÁÉÍÓÚñÑüÜ\s.,!?]", "", text)
    text = re.sub(r"\s+", " ", text)
...
def translate_text_with_deepl(text: str, source_lang: str, target_lang: str = "EN") -> str:
    if not deepl_api_key:
        raise HTTPException(status_code=500, detail="DEEPL_API_KEY is not configured.")
    response = requests.post(
        "https://api-free.deepl.com/v2/translate",
    ...
    if response.status_code != 200:
        raise HTTPException(status_code=response.status_code, detail=response.text)
    result = response.json()
    return result.get("translations", [{}])[0].get("text", "").strip()
...
async def transcribe_audio(file: UploadFile = File(...)):
    ...
    files = {"file": (file.filename, contents, file.content_type or "audio/m4a")}
    data = {"model_id": "scribe_v1"}
    response = requests.post("https://api.elevenlabs.io/v1/speech-to-text", headers=headers, files=files, data=data, timeout=60)
    if response.status_code != 200:
        raise HTTPException(status_code=response.status_code, detail=response.text)
    result = response.json()
    return {"text": clean_transcript(result.get("text", "")), "raw": result}
...
async def translate_and_search(request: TranslateAndSearchRequest):
    ...
    if not raw_text:
        raise HTTPException(status_code=400, detail="Text is required.")
    source_lang = get_source_lang_code(request.language)
    translated_text = translate_text_with_deepl(raw_text, source_lang="ES") if source_lang == "ES" else raw_text
    return {"original_text": raw_text, "translated_text": translated_text, "clinics": MOCK_DATABASE_RESULTS}
